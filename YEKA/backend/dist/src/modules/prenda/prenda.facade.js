...
let PrendaFacade = class PrendaFacade {
    ...
    facturaFacade;
    configService;
    constructor(prendaDAO, prismaService, facturaFacade, configService) {
    ...
    async asignarServicio(prendaId, dto, usuarioId) {
        ...
        if (dto.tipoExpress === client_1.TipoExpress.EXPRESS_48H) {
            const multStr = this.configService.get('EXPRESS_48H_MULTIPLIER') || '1.30';
            multiplier = parseFloat(multStr);
        }
        else if (dto.tipoExpress === client_1.TipoExpress.EXPRESS_24H) {
            const multStr = this.configService.get('EXPRESS_24H_MULTIPLIER') || '1.50';
            multiplier = parseFloat(multStr);
        }
