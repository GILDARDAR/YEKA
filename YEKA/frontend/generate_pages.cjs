...
const generatePage = (name, title, entity, pathName) => `
...
const ${name}Page: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.patch('/${pathName}/' + editingId, formData);
      } else {
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Gestión de ${entity}</p>
        </div>
          <button
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p className="empty-state-title">No hay ${entity} registrados</p>
        </div>
      ) : (
        ...
        <div style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: '400px', padding: 'var(--space-6)' }}>
            <h2 style={{ fontSize: 'var(--text-xl)', fontFamily: 'var(--font-heading)', marginBottom: 'var(--space-4)' }}>
              {editingId ? 'Editar ${title}' : 'Nuevo ${title}'}
            </h2>
...
fs.writeFileSync(path.join(basePath, 'MaterialesPage.tsx'), generatePage('Materiales', 'Materiales', 'materiales', 'material'));
fs.writeFileSync(path.join(basePath, 'TiposArregloPage.tsx'), generatePage('TiposArreglo', 'Tipos de Arreglo', 'tipos de arreglo', 'tipo-arreglo'));
fs.writeFileSync(path.join(basePath, 'ZonasPage.tsx'), generatePage('Zonas', 'Zonas', 'zonas', 'zona'));
