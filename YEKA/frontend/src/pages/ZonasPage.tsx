...
const ZonasPage: React.FC = () => {
  ...
  const handleOpenModal = (item?: Zonas) => {
    if (item) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.patch('/zona/' + editingId, formData);
      } else {
        await api.post('/zona', formData);
      }
      fetchItems();
      handleCloseModal();
    } catch (err: any) {
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Gestión de zonas</p>
        </div>
          <button
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p className="empty-state-title">No hay zonas registrados</p>
        </div>
      ) : (
        ...
        <div style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: '400px', padding: 'var(--space-6)' }}>
            <h2 style={{ fontSize: 'var(--text-xl)', fontFamily: 'var(--font-heading)', marginBottom: 'var(--space-4)' }}>
              {editingId ? 'Editar Zonas' : 'Nuevo Zonas'}
            </h2>
